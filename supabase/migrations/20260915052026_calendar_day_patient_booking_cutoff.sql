-- v1.0.360 — autoagendamento do Portal com corte por dia civil.
-- Um horário permanece reservável até 23:59:59 do dia anterior.
-- Ao virar 00:00 do próprio dia do atendimento em America/Sao_Paulo, o Portal fecha a confirmação.
...
comment on function public.patient_portal_available_slots(text, timestamptz, integer) is
  'Lista horários do Portal somente em datas posteriores ao dia civil atual de America/Sao_Paulo. O parâmetro cutoff_at é mantido por compatibilidade e não define mais janela móvel de 24h.';
...
BEGIN
  ...
  IF (v_slot.starts_at at time zone 'America/Sao_Paulo')::date
     <= (now() at time zone 'America/Sao_Paulo')::date THEN
    RETURN jsonb_build_object(
      'ok', false,
      'code', 'SAME_DAY_CLOSED',
      'error', 'O agendamento pelo Portal encerra à meia-noite do dia do atendimento. Escolha um horário de uma data futura.'
    );
  END IF;
...
comment on function public.book_patient_schedule_slot(text, uuid, uuid, uuid, text, text) is
  'Confirma horário no Portal apenas quando a data do slot é posterior ao dia civil atual em America/Sao_Paulo; mantém vínculo ativo em patient_doctor_links como autorização.';
