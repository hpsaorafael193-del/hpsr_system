...
export default function RecordsPage() {
  ...
  async function handleCreatePatient(data: {
    ...
    if (data.discord.trim() && !isValidDiscordId(data.discord)) return void hpsrAlert("Informe somente o ID numérico do perfil do Discord.", "Discord inválido");
    if (data.discord.trim() && normalizeDiscordId(data.discord) === normalizedPassport.replace(/\D/g, "")) return void hpsrAlert("Esse número é o passaporte/ID da cidade do paciente, não o ID do Discord.", "Contato incorreto");
  ...
  async function handleEditPatient(data: { name: string; passport: string; age: string; birthDate: string; sex: string; bloodType: string; cityPhone: string; discord: string; followUp: string }) {
    ...
    if (data.cityPhone.trim() && !isValidCityPhone(data.cityPhone)) return void hpsrAlert("Use o telefone da cidade no formato (055) 000-000.", "Telefone inválido");
    if (data.discord.trim() && !isValidDiscordId(data.discord)) return void hpsrAlert("Informe somente o ID numérico do perfil do Discord.", "Discord inválido");
    if (data.discord.trim() && normalizeDiscordId(data.discord) === nextPassport.replace(/\D/g, "")) return void hpsrAlert("Esse número é o passaporte/ID da cidade do paciente, não o ID do Discord.", "Contato incorreto");
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex min-w-0 flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                    ...
                    <div className="flex shrink-0 flex-wrap gap-2 xl:max-w-[500px] xl:justify-end">
                      ...
                      {isInternalLinkManager && <button type="button" onClick={() => setIsRecoveryOpen(true)} className="inline-flex min-h-[40px] items-center justify-center gap-2 rounded-[13px] border border-amber-200 bg-amber-50 px-3 text-xs font-black text-amber-800 transition hover:bg-amber-100"><KeyRound size={15} />Recuperar acesso</button>}
          ...
          <section className="rounded-[18px] border border-hpsr-border bg-white p-4 shadow-[0_10px_28px_rgba(79,42,21,0.05)]">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <ModalField label="ID do Discord"><input className={modalInputClass} value={form.discord} onChange={(event) => updateField("discord", normalizeDiscordId(event.target.value))} inputMode="numeric" placeholder="ID numérico do perfil" /></ModalField>
              </div>
