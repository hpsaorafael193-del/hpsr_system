    ...
    <div className="hpsr-page-scroll space-y-3">
      ...
      <section className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-[18px] border border-white/70 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-hpsr-border pb-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-[15px] bg-hpsr-wine text-white"><Activity size={20}/></span><div><h2 className="font-black text-hpsr-text">Atividades do sistema</h2><p className="text-sm text-hpsr-muted">Linha do tempo administrativa consolidada.</p></div></div><label className="flex min-h-[42px] min-w-[280px] items-center gap-3 rounded-[15px] border border-hpsr-border bg-[#fffaf4] px-3"><Search size={17} className="text-hpsr-muted"/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Buscar atividade" className="w-full bg-transparent text-sm font-semibold outline-none"/></label></div>
          <div className="mt-4 grid gap-2">{unified.slice(0,150).map((item)=><article key={item.id} className="grid gap-2 rounded-[15px] border border-hpsr-border bg-[#fffaf4] p-3 sm:grid-cols-[150px_1fr_auto] sm:items-center"><div><p className="text-[10px] font-black uppercase tracking-[.12em] text-hpsr-wineLight">{item.module}</p><p className="mt-1 text-xs text-hpsr-muted">{new Date(item.createdAt).getTime() ? new Date(item.createdAt).toLocaleString("pt-BR") : "Data não registrada"}</p></div><div><p className="text-sm font-black text-hpsr-text">{item.action}</p><p className="mt-1 text-xs leading-relaxed text-hpsr-muted">{item.description}</p></div>{item.reference && <span className="rounded-full border border-hpsr-border bg-white px-2.5 py-1 text-[10px] font-black text-hpsr-wine">{item.reference}</span>}</article>)}{!unified.length&&<div className="rounded-[15px] border border-dashed border-hpsr-border p-8 text-center text-sm text-hpsr-muted">Nenhuma atividade registrada.</div>}</div>
        </div>
