      ...
      <div className="hpsr-modal-motion relative z-10 flex max-h-[92dvh] w-full max-w-6xl flex-col overflow-hidden rounded-[20px] border border-white/50 bg-[#fcf6ee] shadow-[0_32px_100px_rgba(27,10,7,.42)]">
        <div className="border-b border-hpsr-border bg-[linear-gradient(135deg,#fffaf4_0%,#f5e7d8_100%)] p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><p className="text-[10px] font-black uppercase tracking-[.16em] text-hpsr-wineLight">Gestão de candidaturas</p><h2 className="mt-1 text-lg font-black text-hpsr-text">Histórico de formulários</h2><p className="mt-1 text-xs text-hpsr-muted">Revise formulários enviados, confira decisões anteriores e abra qualquer candidatura para análise ou correção.</p></div>
          ...
          <label className="mt-3 flex min-h-[40px] items-center gap-2 rounded-[13px] border border-hpsr-border bg-white px-3"><Search size={15} className="text-hpsr-muted"/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar formulário por nome, passaporte, cargo ou status" className="w-full bg-transparent text-xs font-semibold outline-none"/></label>
        </div>
          ...
          <div className="grid gap-2">
            {visibleItems.map((item, index) => {
              ...
              return <article key={item.protocol || item.id || `${item.passport}-${index}`} className="rounded-[15px] border border-hpsr-border bg-white p-3">
                ...
                {onOpenAnalysis && <button type="button" onClick={() => onOpenAnalysis(item)} className="mt-3 rounded-[12px] border border-hpsr-border bg-[#fff8f0] px-3 py-2 text-xs font-black text-hpsr-wine">Abrir formulário</button>}
              </article>;
            })}
