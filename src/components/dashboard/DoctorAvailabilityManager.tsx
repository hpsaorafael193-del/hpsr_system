...
export function DoctorAvailabilityManager({ doctorId, doctorName, doctorRole = "", defaultSpecialty, embedded = false }: Props) {
  ...
  async function createAvailability() {
    ...
    try {
      ...
      if (endHour * 60 + endMinute < startHour * 60 + startMinute) throw new Error("O último horário não pode ser anterior ao primeiro.");
  ...
  const previewSlots = useMemo(() => {
    ...
    const limit = Math.min(MAX_DAILY_SLOTS, Math.max(1, Number(form.dailyLimit) || MAX_DAILY_SLOTS));
    return Array.from({ length: limit }, (_, index) => start + index * duration)
      .filter((minutes) => minutes <= end)
      .map((minutes) => `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}`);
  }, [form.dailyLimit, form.duration, form.endTime, form.startTime]);
        ...
        <div className="grid gap-4">
          ...
          <div className="rounded-[16px] border border-hpsr-border bg-[#fffaf4] p-3.5">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <label className={label}>Último horário<input type="time" value={form.endTime} onChange={(event) => setForm({ ...form, endTime: event.target.value })} className={field} /></label>
          ...
          <div className="rounded-[16px] border border-hpsr-border bg-white p-3.5">
            <p className="text-[10px] font-black uppercase tracking-[0.12em] text-hpsr-wineLight">Horários que serão publicados</p>
            <div className="mt-2 flex flex-wrap gap-2">{previewSlots.length ? previewSlots.map((time) => <span key={time} className="rounded-full border border-hpsr-border bg-[#fffaf4] px-3 py-1.5 text-xs font-black text-hpsr-wine">{time}</span>) : <span className="text-xs font-semibold text-hpsr-muted">Nenhum horário válido nesta faixa.</span>}</div>
          </div>
