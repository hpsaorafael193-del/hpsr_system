...
type Appointment = {
  ...
  consultationRecords?: Array<{ id: string; type: string; title: string; createdAt: string }>;
};
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          {!compact && <div className="mt-3 rounded-[14px] border border-blue-200 bg-white/80 px-3.5 py-3 text-xs leading-relaxed text-blue-800">
            ...
            <p className="mt-2 font-black text-rose-600">Use o ID numérico. Não use apelido ou nome de usuário.</p>
          </div>}
        </div>
      </div>
    </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-semibold leading-relaxed text-amber-900">Você possui {pendingReschedules.length} consulta{pendingReschedules.length === 1 ? "" : "s"} em ajuste. Você não precisa fazer nada aqui. O médico combina o novo horário com você e ele aparece no Portal quando for definido.</p>
              </div>
            </div>
          </div>
        </section>
